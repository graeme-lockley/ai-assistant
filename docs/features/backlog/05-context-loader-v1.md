...
- **Compose prompt**: From bundle, concatenate core then supporting fragments into the system prompt; enforce a total token budget (e.g. reserve 50% for conversation + response). Truncate or drop lowest-value supporting fragments if over budget.
