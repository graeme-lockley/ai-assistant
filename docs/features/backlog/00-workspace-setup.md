...
- [ ] workspace.template exists and contains the full layout matching workspace-design §3 and context-loader-spec §7.
- [ ] Default workspace root is `~/.ai-assistant.workspace` when no override is set.
- [ ] On startup, if workspace root does not exist, it is created and populated from template.
- [ ] On startup, if workspace root exists, missing directories are created; existing files are not overwritten.
- [ ] Override via `AI_ASSISTANT_WORKSPACE` works; if override path does not exist, populate from template.
- [ ] Server resolves workspace paths from the configured root; file tools respect workspace root.
