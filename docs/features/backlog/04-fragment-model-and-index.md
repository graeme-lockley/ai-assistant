...
- **Index**: Write fragments to `context/indexes/fragments.jsonl` (one JSON object per line). Index build: scan workspace, parse files, emit fragments. Update: on demand (e.g. before loader runs) or when workspace files change (e.g. file watcher or explicit "rebuild index").
...
- **Workspace scaffold**: Workspace and `context/indexes/` exist.
...
- [ ] Index can be rebuilt from current workspace state (CLI or API or on startup).
